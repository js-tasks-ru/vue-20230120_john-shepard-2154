<template>
  <div class="dropdown dropdown_opened">
    <button
      ...
      :class="{ dropdown__toggle_icon: dropdown__toggle_icon_class }"
      :value="modelValue"
    ...
    <div v-show="isDropdownOpen" class="dropdown__menu" role="listbox">
      <button
        ...
        :class="{ dropdown__item_icon: dropdown__toggle_icon_class }"
        role="option"
        type="button"
        :value="option.value"
    ...
    <select id="select-options" name="options" :value="modelValue" @change="selectOption($event.target)">
      <option v-for="option in options" :key="option.value" :value="option.value" @change="selectOption(option)">
        {{ option.text }}
      </option>
    </select>
  </div>
...
export default {
  ...
  computed: {
    selectedOption() {
      return this.options[this.options.findIndex((option) => option.value == this.modelValue)];
    },

    dropdown__toggle_icon_class() {
      return this.options.some((element) => {
        if (element.icon) return true;
        else return false;
      });
    },
  },
