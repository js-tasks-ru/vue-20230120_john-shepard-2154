  ...
  <form class="meetup-form" @submit.prevent="handleSubmit">
    <div class="meetup-form__content">
      ...
      <meetup-agenda-item-form
        ...
        @update:agenda-item="updateAgendaItem"
...
export default {
  ...
  data() {
    return {
      // localMeetup: { ...this.meetup, agenda: lodashCloneDeep(this.meetup.agenda) },
      localMeetup: lodashCloneDeep(this.meetup),
    };
  },
  ...
  methods: {
    addAgendaItem() {
      ...
      if (this.localMeetup.agenda.length > 0) agenda.startsAt = lodashLast(this.localMeetup.agenda).endsAt;
      this.localMeetup.agenda.push(agenda);
    },

    updateAgendaItem(changedAgendaItem) {
      this.localMeetup.agenda[this.localMeetup.agenda.length - 1] = changedAgendaItem;
    },

    removeAgendaItem(agendaItemId) {
      lodashRemove(this.localMeetup.agenda, function (agendaItem) {
    ...
    handleSubmit() {
      this.$emit('submit', lodashCloneDeep(this.localMeetup));
    },
  },
};
</script>
